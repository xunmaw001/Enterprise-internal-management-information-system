...
//                            bumenEntity.setCreateTime(date);//时间
    ...
    */
    ...
    public R add(@RequestBody BumenEntity bumen, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,bumen:{}",this.getClass().getName(),bumen.toString());
        Wrapper<BumenEntity> queryWrapper = new EntityWrapper<BumenEntity>()
            .eq("bumen_uuid_number", bumen.getBumenUuidNumber())
            .eq("bumen_name", bumen.getBumenName())
            .eq("bumen_address", bumen.getBumenAddress())
//            .notIn("bumen_types", new Integer[]{102})
            ;
        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        BumenEntity bumenEntity = bumenService.selectOne(queryWrapper);
        if(bumenEntity==null){
