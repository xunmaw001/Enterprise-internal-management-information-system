...
, "jingliId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setYonghuId(yonghu.getId());
            }
            //级联表 部门经理
            //级联表
            JingliEntity jingli = jingliService.selectById(qingjia.getJingliId());
            if(jingli != null){
            BeanUtils.copyProperties( jingli , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"
, "jingliId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
    ...
    */
    ...
    public R save(@RequestBody QingjiaEntity qingjia, HttpServletRequest request){
        ...
        if(false)
            ...
            qingjia.setJingliId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<QingjiaEntity> queryWrapper = new EntityWrapper<QingjiaEntity>()
            .eq("yonghu_id", qingjia.getYonghuId())
            .eq("jingli_id", qingjia.getJingliId())
...
//                            qingjiaEntity.setJingliId(Integer.valueOf(data.get(0)));   //部门主管 要改的
    ...
    public R detail(@PathVariable("id") Integer id, HttpServletRequest request){
        ...
        QingjiaEntity qingjia = qingjiaService.selectById(id);
            if(qingjia !=null){
                ...
                if(yonghu != null){
                    BeanUtils.copyProperties( yonghu , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"
, "jingliId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setYonghuId(yonghu.getId());
                }
                //级联表
                    JingliEntity jingli = jingliService.selectById(qingjia.getJingliId());
